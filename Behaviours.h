...
void CombatGridLineTraceBehaviour<type>::loop(float delta) {
	...
	if      (points[idx].second - y < 0) y = max((float)points[idx].second, y - 1.2f*modif*quantity);
	else if (points[idx].second - y > 0) y = min((float)points[idx].second, y + 1.2f*modif*quantity);
