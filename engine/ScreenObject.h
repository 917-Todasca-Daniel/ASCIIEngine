...
public:
	TextFloatingPopup(std::string text, float lifespan, float speed, int x, int y, int priority = 100) : orx(x), ory(y), maxtime(lifespan), speed(speed), amount(0), ScreenObject(x, y, priority, "") { 
		updateShape(text); 
		color = DRED;
		show();
		setName("floating");
